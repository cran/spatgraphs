...
void Graph::Init(Pp *pp0, int *gtype0, double *par0, double *prepR0, int *doDists0,
		double *preDists, int *toroidal0, int *inc0, double *wMatrix, int *dbg0, SEXP *other0, SEXP *env0)
{
	int typein,i,j;
...
void Graph::sg_calc()
{
	...
	else if(*gtype==13) this->sg_random();
	preEdges = 1;
}
...
/********************************************************************************************/
// Random graph: P(i <-> j) = f(dij, mi, mj)
SEXP mkvar(double a)
{
	SEXP ans;
...
void Graph::sg_random()
{
	if(*dbg) Rprintf("Random edges: ");
	int i,j;
	double u, pij;
//	defineVar(install("dij"), mkvar(0), *env);
//	defineVar(install("mi"), mkvar(0), *env);
//	defineVar(install("mj"), mkvar(0), *env);
	for(i=0; i<pp->size()-1; i++)
		for(j=i+1; j<pp->size(); j++)
		{
			u = unif_rand();
			defineVar(install("dij"), mkvar(pp->getDist(&i, &j)), *env);
			defineVar(install("mi"), mkvar(pp->getMass(&i)), *env);
			defineVar(install("mj"), mkvar(pp->getMass(&j)), *env);
			pij = REAL(eval(*other, *env))[0];
			if(u < pij){
				addNew(i, j+1);
				addNew(j, i+1);
			}
